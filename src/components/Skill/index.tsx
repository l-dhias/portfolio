import { SkillImg, ContainerSkill } from './styles'
import { Paragrafo } from '../Paragrafo'

export const HTML = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">HTML</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/html5/html5-original.svg" />
  </ContainerSkill>
)

export const CSS = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">CSS</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/css3/css3-original.svg" />
  </ContainerSkill>
)
export const Less = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Less</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/less/less-plain-wordmark.svg" />
  </ContainerSkill>
)
export const Sass = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Sass</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/sass/sass-original.svg" />
  </ContainerSkill>
)
export const StyledComponents = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Styled</Paragrafo>
    <Paragrafo tipo="secundario">Components</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/styledcomponents/styledcomponents-plain.svg" />
  </ContainerSkill>
)
export const Gulp = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Gulp</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/gulp/gulp-plain.svg" />
  </ContainerSkill>
)
export const Grunt = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Grunt</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/grunt/grunt-line.svg" />
  </ContainerSkill>
)
export const Figma = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Figma</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/figma/figma-original.svg" />
  </ContainerSkill>
)
export const Babel = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Babel</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/babel/babel-original.svg" />
  </ContainerSkill>
)
export const JQuery = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">JQuery</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/jquery/jquery-original.svg" />
  </ContainerSkill>
)
export const Bootstrap = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Bootstrap</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/bootstrap/bootstrap-original.svg" />
  </ContainerSkill>
)
export const JavaScript = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">JavaScript</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-plain.svg" />
  </ContainerSkill>
)
export const TypeScript = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">TypeScript</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-plain.svg" />
  </ContainerSkill>
)
export const NodeJs = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">NodeJs</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/nodejs/nodejs-original-wordmark.svg" />
  </ContainerSkill>
)
export const Redux = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Redux</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/redux/redux-original.svg" />
  </ContainerSkill>
)
export const React = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">React</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original.svg" />
  </ContainerSkill>
)
export const Vue = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Vue</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/vuejs/vuejs-original.svg" />
  </ContainerSkill>
)
export const VsCode = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">VsCode</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/vscode/vscode-original.svg" />
  </ContainerSkill>
)
export const GitHub = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">GitHub</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/github/github-original.svg" />
  </ContainerSkill>
)
export const Git = () => (
  <ContainerSkill>
    <Paragrafo tipo="secundario">Git</Paragrafo>
    <SkillImg src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg" />
  </ContainerSkill>
)
